...
public class UpdateItemCommandHandler(AppDbContext db, IActivityLogger activityLogger, ILogger<UpdateItemCommandHandler> logger)
    : IRequestHandler<UpdateItemCommand, TodoItemResponse>
{
    public async Task<TodoItemResponse> Handle(UpdateItemCommand request, CancellationToken ct)
    {
        ...
        {
            var newStatusName = ((ItemStatus)item.StatusId).ToDisplayName();
            activityLogger.Log(request.ListId, request.ProfileId, "StatusChanged",
                $"{ActivityLogger.Truncate(item.Content)} → {newStatusName}",
                item.UpdatedAt);
            logger.LogDebug("Item {ItemId} status changed from {Previous} to {New} by profile {ProfileId}",
                item.Id, (ItemStatus)previousStatus, (ItemStatus)item.StatusId, request.ProfileId);
        }
        ...
        {
            activityLogger.Log(request.ListId, request.ProfileId, "NotesUpdated",
                ActivityLogger.Truncate(item.Content),
                item.UpdatedAt);
        }

        await db.SaveChangesAsync(ct);

        logger.LogDebug("Item {ItemId} updated in list {ListId} by profile {ProfileId}",
